...
function loginUser(event) {
    event.preventDefault(); // Evita o comportamento padrão do form
     ...
     .then((data) => {
        if (data.message); // Exibe mensagem de sucesso
        window.location.href = "dashboard.html"; // Redireciona para a dashboard
     })
     .catch((error) => {
        console.error(error); 
        alert("Usuários ou senha inválidos. Tente novamente.");
     });
}
...
function registerUser(event) {
    event.preventDefault(); // Evita o comportamento padrão de envio de form
...
// Adiciona os eventos aos form, dependendo da página
...
if (window.location.pathname.includes("dashboard.html")) {
    loadUser(); // Carrega os usuários ao carregar a página
}
